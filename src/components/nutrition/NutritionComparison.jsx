    ...
    <div className="space-y-6">
      ...
      <div className="surface p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <MacroBar label="Protein" logged={t.pro} target={targets.pro} color={MACRO_COLORS.protein} />
          <MacroBar label="Carbs" logged={t.carb} target={targets.carb} color={MACRO_COLORS.carbs} />
          <MacroBar label="Fat" logged={t.fat} target={targets.fat} color={MACRO_COLORS.fat} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[hsl(var(--brand)/0.05)] border border-[hsl(var(--brand)/0.1)] text-[12px] text-[hsl(var(--brand))]">
        ...
        <span className="font-medium">
          {targets.cal === 0 ? 'Set your calorie target in your profile' : calPct >= 90 ? '✓ Calorie target reached' : `${Math.round(calPct)}% of calorie target`}
        </span>
      </div>
    </div>
  );
