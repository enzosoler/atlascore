...
const CARD_CLASS_NAME =
  'rounded-[20px] border border-[hsl(var(--border)/0.9)] bg-[hsl(var(--card)/0.9)] shadow-[var(--shadow-sm)]';

export function AppContainer({ children, className = '' }) {
  return (
    <div className="atlas-page-shell">
      <div className={cn('mx-auto flex w-full max-w-[30rem] flex-col gap-5 px-4 pb-8 pt-5 sm:px-5 sm:pb-10', className)}>
        {children}
      </div>
    </div>
  );
...
export function PageHeader({
  ...
  children,
  className = '',
  accentClassName = 'from-[hsl(var(--brand)/0.08)]',
}) {
  return (
    <Card className={cn('relative overflow-hidden px-5 py-5', className)}>
      ...
      <div className="relative space-y-5">
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            <h1 className="atlas-display-title text-[2.15rem] sm:text-[2.35rem]">{title}</h1>
            {subtitle ? <p className="atlas-section-description">{subtitle}</p> : null}
          </div>
        </div>
      ...
      {eyebrow || title || subtitle || actions ? (
        <div className={cn('flex items-end justify-between gap-3', headerClassName)}>
          <div className="min-w-0 space-y-2">
            {eyebrow ? <p className="atlas-overline">{eyebrow}</p> : null}
            {title ? <h2 className="atlas-section-title text-[1.125rem]">{title}</h2> : null}
            {subtitle ? <p className="atlas-section-description">{subtitle}</p> : null}
          </div>
  ...
  return (
    <Card className={cn('px-4 py-4', className)}>
      <div className="flex items-start justify-between gap-3">
        ...
        {Icon ? (
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[20px] border border-[hsl(var(--border)/0.82)] bg-[hsl(var(--fill)/0.8)] text-[hsl(var(--fg-2))]">
...
export function TabBar({ items, className = '' }) {
  return (
    <nav
      className={cn(
        'glass fixed inset-x-0 bottom-0 z-[60] border-t border-[hsl(var(--border)/0.72)] lg:hidden',
        className
      )}
