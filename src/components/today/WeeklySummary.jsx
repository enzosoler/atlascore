...
export default function WeeklySummary({ allCheckins, meals, workouts }) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      date,
      dateStr,
      dayLabel: t(`today.weeklySummary.days.${DAY_KEYS[date.getDay()]}`),
      checkin: dayCheckin,
  ...
  return (
    <div className="atlas-card p-4 space-y-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <p className="atlas-overline">{t('today.weeklySummary.label')}</p>
      </div>

      <div className="flex items-end justify-between gap-1.5 h-20">
        {days.map((day) => {
          const heightPct = (day.score / 100) * 100;
          const color = day.score >= 80
            ? 'bg-[hsl(var(--ok))]'
            : day.score >= 50
              ? 'bg-[hsl(var(--warn))]'
              : 'bg-[hsl(var(--fg-2)/0.2)]';
      ...
      <div className="grid grid-cols-3 gap-2 pt-2 border-t border-[hsl(var(--border-h))]">
        <div className="text-center">
          <p className="text-[11px] text-[hsl(var(--fg-2))] mb-0.5">{t('today.weeklySummary.average')}</p>
          <p className="text-[14px] font-bold text-[hsl(var(--fg))]">{weekAvgScore}%</p>
        </div>
        <div className="text-center">
          <p className="text-[11px] text-[hsl(var(--fg-2))] mb-0.5">{t('today.weeklySummary.checkins')}</p>
          <p className="text-[14px] font-bold text-[hsl(var(--fg))]">{checkinsThisWeek}/7</p>
        </div>
        <div className="text-center">
          <p className="text-[11px] text-[hsl(var(--fg-2))] mb-0.5">{t('today.weeklySummary.workouts')}</p>
