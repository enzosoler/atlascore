...
const STATUS_META = {
  pending: {
    label: 'Pendente',
    chip: 'border-[hsl(var(--warn)/0.18)] bg-[hsl(var(--warn)/0.12)] text-[hsl(34_68%_32%)]',
...
function PlanSourceBadge({ source }) {
  const meta = PLAN_SOURCES[source] || PLAN_SOURCES.self;
      ...
      <div className="rounded-[28px] border border-dashed border-[hsl(var(--border)/0.8)] bg-[hsl(var(--fill)/0.3)] px-6 py-8 text-center">
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          ...
          <button
            type="button"
            className={cn(
              'atlas-button inline-flex h-9 items-center gap-2 border px-4 text-[12px]',
              PLAN_SOURCES.ai.className
            )}
          >
          ...
          <button
            ...
            className={cn(
              'atlas-button inline-flex h-9 items-center gap-2 border px-4 text-[12px]',
              PLAN_SOURCES.coach.className
            )}
          >
            <UserCheck className="h-3.5 w-3.5" strokeWidth={1.9} />
            Ask your coach
          </button>
        </div>
      </div>
    ...
    <div className="rounded-[28px] border border-[hsl(var(--border)/0.9)] bg-[hsl(var(--card)/0.82)] px-5 py-5 shadow-[var(--shadow-xs)]">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-3 text-[1.25rem] font-semibold tracking-[-0.035em] text-[hsl(var(--fg))]">
            {plan.name}
          </p>
          <p className="mt-1 text-[13px] leading-6 text-[hsl(var(--fg-2))]">
            {plan.exercises.length} exercises · {plan.duration_minutes || 0} min · RPE{' '}
            {plan.perceived_effort || '--'}
          </p>
        </div>
        <button
          type="button"
          onClick={onLogSession}
          className="atlas-button atlas-button-primary inline-flex h-10 shrink-0 items-center gap-2 px-5 text-[13px]"
        >
...
export default function Workouts() {
  return (
    <SafePageBoundary
      title="Treinos"
      subtitle="Plano de treino e treinos livres separados em dois espaços distintos."
      maxWidth="max-w-6xl"
...
function WorkoutsContent() {
  ...
  return (
    <AppContainer>
      ...
      >
        <PlanCard plan={plannedWorkout} onLogSession={handleLogPlanSession} />
        ...
        {planSessions.length > 0 ? (
          <div className="mt-4 space-y-4">
            ...
            {planSessions.map((workout, index) => (
              <div key={workout.id} className="space-y-4">
                {index > 0 ? (
                  <div className="mx-auto h-px w-full max-w-[92%] bg-[hsl(var(--border)/0.75)]" />
                ) : null}
                <WorkoutCard
                  workout={workout}
      ...
      >
        ...
        {filteredFreeWorkouts.length > 0 ? (
          <div className="space-y-4">
            {filteredFreeWorkouts.map((workout, index) => (
              <div key={workout.id} className="space-y-4">
                {index > 0 ? (
                  <div className="mx-auto h-px w-full max-w-[92%] bg-[hsl(var(--border)/0.75)]" />
                ) : null}
