...
export default function V3Privacy() {
  ...
  const sections = getTranslation('legal.privacy.sections') || [];
  return (
    <V3MarketingLayout
      eyebrow={t('legal.privacy.eyebrow')}
      title={<>{t('legal.privacy.titlePrefix')} <span style={{ color: MC.accent }}>{t('legal.privacy.titleAccent')}</span></>}
      intro={`${t('legal.privacy.introPrefix')} ${t('legal.lastUpdated')}`}
      hideCTAs
    >
          ...
          <Section key={section.n} n={section.n} title={section.title}>
            {(section.paragraphs || []).map((paragraph, index) => (
              <P key={`${section.n}-p-${index}`}>
                {paragraph}
                {section.email && !section.emailLead && index === section.paragraphs.length - 1 ? (
                  <>
                    {' '}
