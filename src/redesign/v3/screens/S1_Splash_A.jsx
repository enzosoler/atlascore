      ...
      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 14 }}>
            <ACLabel size={13} color={c.dim} track={0}>{t('welcome.tagline')}</ACLabel>
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <ACBtn primary block dark={dark} size="lg" pill onClick={onGetStarted}>{t('welcome.getStarted')}</ACBtn>
        <div style={{ textAlign: 'center', padding: '8px 0' }}>
          ...
          >
            <ACLabel size={13} color={c.dim}>
              {t('welcome.memberPrompt')} <span style={{ color: c.fg, fontWeight: 600 }}>{t('welcome.signIn')}</span>
            </ACLabel>
          </button>
        </div>
      </div>
