 ...
 * Gift moment: streak / adherence update (§12). Plays 3-stage reveal on mount.
 *
 ...
 *               onClose={fn} onGoToday={fn} onShare={fn} />
 *
 * stats: [{k, v, sub, hi?}]
 * dailyBars: [{d, v, hi?}] — v is 0–100 (relative %)
 * moments: [{d, t, hi?}]
 * onShare({weekLabel, issueNumber, stats, heroLines, sharedAt}) — share recap card
 */
...
export default function S30_Weekly_Recap({
  ...
  // 3-stage gift moment reveal (§12: streak/adherence update)
  // phase 0 = anticipation, 1 = reveal (hero+stats), 2 = full (bars+moments+kicker)
  ...
  const handleShare = () => {
    if (typeof onShare === 'function')
      onShare({ weekLabel, issueNumber, stats: _stats, heroLines, sharedAt: Date.now() });
  };
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: c.ink, color: '#efe9da' }}>
      <div style={{ padding: '14px 22px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button type="button" onClick={handleShare} style={{
          ...
          background: 'rgba(239,233,218,0.08)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          border: 'none', cursor: onShare ? 'pointer' : 'default', opacity: onShare ? 1 : 0.35,
        }}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            <path d="M3 5l4-4 4 4M7 1v9M2 13h10" stroke="#efe9da" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>
      </div>

      <div style={{ flex: 1, overflow: 'auto', WebkitOverflowScrolling: 'touch', padding: '22px 22px 30px', position: 'relative' }}>
        {/* Anticipation overlay — phase 0 only, non-noData path */}
        {phase === 0 && !noData && (
          <div style={{
            ...
            <div style={{
              ...
              border: '2px solid rgba(239,233,218,0.25)',
              borderTopColor: '#efe9da',
              animation: 'spin 0.9s linear infinite',
            }} />
            <div style={{
              marginTop: 16, fontFamily: ACFonts.mono, fontSize: 11,
              color: 'rgba(239,233,218,0.65)', letterSpacing: 0.8, textTransform: 'uppercase',
            }}>
              Building your week...
            </div>
        ...
        {noData ? (
          ...
          <>
            {/* Stage 1 reveal: hero title + stats (phase >= 1) */}
            <div style={{
              opacity: phase >= 1 ? 1 : 0,
            ...
            {/* Stage 2 celebration: bars + moments + kicker (phase >= 2) */}
            <div style={{
              opacity: phase >= 2 ? 1 : 0,
