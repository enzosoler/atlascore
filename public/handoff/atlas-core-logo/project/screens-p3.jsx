...
function CompareTile({ label, c, dark, dim }) {
  // non-dim tile bg is c.fg → silhouette must use c.bg-alpha to contrast.
  const fgTint1 = dark ? 'rgba(10,10,10,0.22)' : 'rgba(239,233,218,0.22)';
  const fgTint2 = dark ? 'rgba(10,10,10,0.18)' : 'rgba(239,233,218,0.18)';
  return (
    <div style={{
...
function PhotoCard({ m, c, dark }) {
  // silhouette paths sit on a c.fg card, so they must be tinted with c.bg (the opposite token).
  const fill1 = dark ? 'rgba(10,10,10,0.22)' : 'rgba(239,233,218,0.22)';
  const fill2 = dark ? 'rgba(10,10,10,0.18)' : 'rgba(239,233,218,0.18)';
  return (
    <div style={{
      aspectRatio: '3/4',
      background: c.fg,
      ...
      <svg width="100%" height="100%" viewBox="0 0 100 140" fill="none"
        style={{ position: 'absolute', top: 0, left: 0 }}>
        <circle cx="50" cy="30" r="12" fill={fill1} />
        <path d="M34 42 L66 42 L62 88 L50 94 L38 88 Z" fill={fill1} />
        <path d="M34 42 L26 82 L30 84 L38 48 Z" fill={fill2} />
        <path d="M66 42 L74 82 L70 84 L62 48 Z" fill={fill2} />
        <path d="M40 92 L36 138 L46 140 L50 100 Z" fill={fill1} />
        <path d="M60 92 L64 138 L54 140 L50 100 Z" fill={fill1} />
      </svg>
      {m.latest && (
        <div style={{
